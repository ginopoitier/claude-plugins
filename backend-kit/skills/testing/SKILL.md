...
name: testing
description: >
  Testing strategy for .NET applications. Covers xUnit v3, WebApplicationFactory
  for integration tests, Testcontainers for real database testing, Verify for
  snapshot testing, and the AAA pattern.
  Load this skill when: "test", "xUnit", "WebApplicationFactory", "Testcontainers",
  "integration test", "unit test", "snapshot test", "Verify", "test coverage",
  "AAA pattern", "WireMock", "FakeTimeProvider", "bUnit", "write tests",
  "test infrastructure", "test data builder".
user-invocable: true
argument-hint: "[feature or test type to write]"
...
5. **Error paths are as important as happy paths** — Every handler test must include at least one test for each failure mode: NotFound, Validation failure, Conflict, Unauthorized.
...
```csharp
// BAD — static shared state causes flaky tests (order-dependent failures)
...
|----------|---------------|
| Testing an API endpoint end-to-end | `WebApplicationFactory` integration test |
| Testing business logic in isolation | Unit test with fakes/stubs, no mocks |
| Database-dependent tests | Testcontainers (real DB), never `UseInMemoryDatabase` |
...
| Shared expensive fixture (container) | `IClassFixture<T>` with `IAsyncLifetime` |
