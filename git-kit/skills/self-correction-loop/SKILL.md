...
## Data Access
- Always use the project's caching layer — never raw in-memory dicts
- Use the project's data access pattern — check CLAUDE.md for conventions
...
User: "No, use the project logger here, not console.log"
Claude: "Fixed. Added to Memory > Code Style:
         Always use the project logger — never console.log in production code."
*Next session: checks memory, uses logger from the start*
```
...
"In the payments handler on line 47, use the structured logger"
...
| Correction is universal (any language) | Store in MEMORY.md — it applies to this project |
