...
name: configuration
description: >
  Configuration patterns for .NET applications. Covers the Options pattern,
...
user-invocable: true
argument-hint: "[configuration class or section to set up]"
...
builder.Services.AddOptions<JwtOptions>()
    ...
    .Validate(options =>
    {
        if (string.IsNullOrEmpty(options.Key) || options.Key.Length < 32)
            return false;
        return options.ExpirationMinutes > 0;
...
// GOOD — appsettings.json has structure only, secrets via user-secrets or env vars
{
