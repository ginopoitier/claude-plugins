...
name: openapi
description: >
  Built-in OpenAPI support for .NET applications. Covers document generation,
...
user-invocable: true
argument-hint: "[endpoint or document to configure]"
...
1. **Built-in, not Swashbuckle** — .NET 10 ships `Microsoft.AspNetCore.OpenApi` as the official solution. Swashbuckle was removed from templates in .NET 9 and is no longer recommended.
...
4. **Metadata on every endpoint** — Use `.WithName()`, `.WithSummary()`, `.WithTags()` on every endpoint.
...
```csharp
// BAD — removed from .NET 9+ templates
