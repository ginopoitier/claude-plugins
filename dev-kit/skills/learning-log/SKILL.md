...
5. **Distinct from handoff notes** — The session-management handoff captures session state (done/pending/learned). The learning log is a persistent, growing knowledge base. Handoffs are overwritten; the log only grows (and is periodically pruned).
...
Primary constructor parameters are implicitly captured as fields.
