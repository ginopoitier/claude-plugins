...
description: >
  ...
  session left off.
  Load this skill when: "wrap up", "let's stop here", "that's all for now",
  "end of session", "save progress", "handoff", "I'm done for today",
  "pick this up tomorrow", "session end", "good enough for now".
user-invocable: true
argument-hint: "[--quick]"
...
- [x] Fixed N+1 query in GetOrderDetails
  ...
  - Changed `.Include()` to `.Select()` projection — removed navigation loading
...
# (Use Write tool — always overwrites, never appends)
```
...
```
// BAD — creating timestamped handoff files, unclear which is current
.claude/
...
| `.claude/` directory doesn't exist | Create it with `mkdir -p .claude` before writing |
