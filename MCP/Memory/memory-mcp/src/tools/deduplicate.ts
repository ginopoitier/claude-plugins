...
function extractConflictRoots(text: string): string[] {
...
export function registerDeduplicateTools(server: McpServer, storage: StorageService): void {
  server.tool(
    "memory_deduplicate",
    "Find near-duplicate memories using cosine similarity. Groups similar pairs and flags potential conflicts where files make opposing claims.",
        ...
        const resultGroups = groups.map((group) => {
          ...
          for (let gi = 0; gi < group.length && !conflict; gi++) {
            for (let gj = gi + 1; gj < group.length && !conflict; gj++) {
              ...
              if (hasConflict(body1, body2)) conflict = true;
            }
          }
