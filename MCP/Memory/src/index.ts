...
async function runCli(): Promise<void> {
  ...
  cli.command(
    ...
    (y) =>
      y
        ...
        .option("min-score", { alias: "s", type: "number", default: 0.2, description: "Minimum score" }),
    async (argv) => {
      const projectId = detectProjectId(argv.project as string | undefined);
      ...
      const minScore = (argv["min-score"] as number) ?? 0.2;
      const limit = (argv.limit as number) ?? 5;

      const results = docs
        .map((doc, i) => ({ memory: memories[i]!, score: scoreQuery(argv.query as string, doc, docs) }))
  ...
  cli.command(
    "health",
    "Check memory health for a project",
    (y) =>
      y.option("project", { alias: "p", type: "string", description: "Project ID" }),
    async (argv) => {
      const projectId = detectProjectId(argv.project as string | undefined);
      ...
      const issues: Array<{ severity: string; file: string; issue: string }> = [];
      ...
      if (byType.user === 0) coverageGaps.push("No user memories");
      if (byType.reference === 0) coverageGaps.push("No reference memories");
      ...
      console.log(
        JSON.stringify(
          { total: memories.length, by_type: byType, issues, coverage_gaps: coverageGaps, score, project_id: projectId },
          null,
          2
        )
      );
  ...
  cli.command(
    "sync-index",
    "Sync MEMORY.md index with actual memory files",
    (y) =>
      y.option("project", { alias: "p", type: "string", description: "Project ID" }),
    async (argv) => {
      const projectId = detectProjectId(argv.project as string | undefined);
      ...
      for (const line of indexContent.split("\n")) {
        ...
        if (match) indexed.set(`memory/${match[1]}`, line);
      }
      ...
      for (const relPath of actualFiles) {
        if (indexed.has(relPath)) unchanged.push(relPath);
        else added.push(relPath);
      }
      for (const relPath of indexed.keys()) {
        if (!actualFiles.has(relPath)) removed.push(relPath);
      }
      ...
      console.log(
        JSON.stringify(
          { added, removed, unchanged, index_path: storage.getIndexPath(projectId), project_id: projectId },
          null,
          2
        )
      );
  ...
  cli.command(
    ...
    (y) =>
      y.option("content", { alias: "c", type: "string", demandOption: true, description: "Content to classify" }),
    async (argv) => {
      ...
      const scores: Record<MemoryType, number> = { feedback: 0, user: 0, project: 0, reference: 0 };
      for (const [type, patterns] of Object.entries(PATTERNS) as [MemoryType, RegExp[]][]) {
        for (const pattern of patterns) {
          pattern.lastIndex = 0;
          const count = (content.match(pattern) ?? []).length;
          if (count > 0) scores[type] += 0.4 * Math.min(count, 3);
        }
        scores[type] = Math.min(scores[type], 1.0);
      }

      let bestType: MemoryType = "feedback";
      let bestScore = 0;
      for (const [t, s] of Object.entries(scores) as [MemoryType, number][]) {
        if (s > bestScore) { bestScore = s; bestType = t; }
      }
      if (bestScore < 0.3) { bestType = "feedback"; bestScore = 0.3; }
      ...
      console.log(
        JSON.stringify(
          { type: bestType, confidence: Math.round(bestScore * 100) / 100, suggested_name, suggested_description },
          null,
          2
        )
      );
