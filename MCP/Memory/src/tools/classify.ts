...
const RULES: Record<MemoryType, ClassifyRule[]> = {
  feedback: [
...
function scoreType(content: string, rules: ClassifyRule[]): number {
  let score = 0;
...
function suggestDescription(content: string): string {
  const trimmed = content.trim();
  const firstSentence = trimmed.split(/[.!?]/)[0]?.trim() ?? trimmed;
  return firstSentence.slice(0, 100);
}

export function registerClassifyTools(server: McpServer): void {
  server.tool(
    "memory_classify",
    "Classify text content into a memory type (user/feedback/project/reference) using heuristic pattern matching. Returns type, confidence, reasoning, and suggested name/description.",
    ClassifyInputSchema,
    async (params) => {
      try {
