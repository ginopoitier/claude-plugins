...
export function registerStoreTools(server, storage) {
    // memory_store
    server.tool("memory_store", "Create or update a memory file. Auto-derives filename from type and name if not specified. Updates the MEMORY.md index automatically.", StoreInputSchema, async (params) => {
        try {
    ...
    // memory_get
    server.tool("memory_get", "Read a memory file by relative path and return all its frontmatter fields and body content.", GetInputSchema, async (params) => {
        try {
            const projectId = detectProjectId(params.project_id);
    ...
    // memory_delete
    server.tool("memory_delete", "Delete a memory file and remove its entry from the MEMORY.md index.", DeleteInputSchema, async (params) => {
        try {
            const projectId = detectProjectId(params.project_id);
            let relPath = params.file_path;
            if (relPath.startsWith("/") || relPath.match(/^[A-Za-z]:\\/)) {
