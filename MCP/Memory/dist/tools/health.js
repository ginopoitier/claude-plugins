...
function indexedFiles(indexContent) {
    const entries = new Set();
    for (const line of indexContent.split("\n")) {
        const match = line.match(/\(memory\/([^)]+\.md)\)/);
        if (match)
            entries.add(`memory/${match[1]}`);
    }
    return entries;
}
export function registerHealthTools(server, storage) {
    server.tool("memory_health", "Analyse memory health for a project: counts by type, missing fields, stale entries, duplicates, index sync status, coverage gaps, and overall score.", HealthInputSchema, async (params) => {
        try {
            ...
            for (const f of actualFiles) {
                if (!indexed.has(f))
                    driftDetails.push(`${f} not in index`);
            }
            for (const f of indexed) {
                if (!actualFiles.has(f))
                    driftDetails.push(`${f} in index but file missing`);
            }
            const indexSync = driftDetails.length === 0 ? "ok" : "drift";
            // Coverage gaps
            const coverageGaps = [];
            if (byType.user === 0)
                coverageGaps.push("No user memories (role/preferences not captured)");
            if (byType.reference === 0)
                coverageGaps.push("No reference memories (no system/tool pointers)");
            // Score calculation
